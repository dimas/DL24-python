...
**UPDATE 2026-06-15**: I haven't touched this thing for a long time. Turns out there are now other implementations available as well as protocol
being much better documented.
* https://github.com/tshaddack/dl24
* https://www.improwis.com/projects/sw_dl24/
...
# Limitations
* Only constant current mode is really supported. I have no idea what commands need to be sent to DL24 in order to switch to other modes (constant power, constant voltage etc) and also how to set target power/voltage/etc for these. So even though you can turn load on and off with your script, you need to manually configure everything.
...
* The whole idea of sending a command to device and waiting for a certain signature in the response data is not great - I saw data packets I cannot explain. But doing it the right way requires some proper documentation on the device protocol rather than guesswork.
